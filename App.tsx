      ...
      <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-slate-200 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3 cursor-pointer" onClick={resetProject}>
            ...
            <div>
              <h1 className="text-xl font-black text-slate-800 tracking-tight">AI Project Mentor</h1>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Architectural Orchestrator</p>
            </div>
          </div>
            ...
            <div className="flex items-center gap-4">
              <div className="flex bg-slate-100 p-1 rounded-xl">
                <button
                  ...
                  className={`px-4 py-1.5 text-xs font-bold rounded-lg transition-all ${activeTab === 'analysis' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}
                >
                  ANALYSIS
                </button>
                <button
                  onClick={() => setActiveTab('chat')}
                  className={`px-4 py-1.5 text-xs font-bold rounded-lg transition-all ${activeTab === 'chat' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}
                >
                  MENTOR CHAT
                </button>
              </div>
              <button onClick={resetProject} className="text-xs font-bold text-red-500 hover:bg-red-50 px-4 py-2 rounded-xl transition-colors">RESET</button>
            </div>
          )}
        </div>
      </header>
        ...
        {currentStep === 'idle' ? (
          <div className="mt-12">
            <div className="text-center mb-16 animate-in fade-in slide-in-from-top-4 duration-700">
              <span className="px-4 py-1.5 bg-blue-50 text-blue-600 rounded-full text-[10px] font-black uppercase tracking-[0.2em] mb-4 inline-block border border-blue-100">Deep Reasoning Engine</span>
              <h2 className="text-5xl font-black text-slate-900 mb-6 tracking-tighter leading-tight">Project Intelligence<br/>Through Chain of Thought.</h2>
          ...
          <div className="max-w-xl mx-auto py-32 text-center space-y-8 animate-in zoom-in duration-300">
            <div className="w-20 h-20 bg-red-50 text-red-600 rounded-3xl mx-auto flex items-center justify-center border border-red-100">
              <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
            </div>
            <div>
              <h3 className="text-2xl font-black text-slate-800 mb-2">Analysis Interrupted</h3>
              <p className="text-slate-500">Something went wrong during the analysis phase. This is often due to context length limits or API rate limits.</p>
            </div>
            ...
            <div className="flex flex-col items-center justify-center space-y-8">
              <div className="relative w-40 h-40">
                <svg className="w-full h-full transform -rotate-90">
                  <circle cx="80" cy="80" r="75" stroke="currentColor" strokeWidth="8" fill="transparent" className="text-slate-100" />
                ...
                <div className="absolute inset-0 flex items-center justify-center flex-col">
                  <span className="text-3xl font-black text-slate-800 leading-none">{getProgressPercentage()}%</span>
                  <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest mt-1">Processing</span>
                </div>
              </div>

              <div className="text-center space-y-2">
                <h3 className="text-3xl font-black text-slate-800 tracking-tight">Orchestrating Gemini 3 Pro</h3>
                <p className="text-slate-500 font-medium">Phase {['understanding', 'architecture', 'improvements', 'documentation', 'pitching'].indexOf(currentStep) + 1} of 5</p>
              </div>
            </div>
              ...
              {['understanding', 'architecture', 'improvements', 'documentation', 'pitching'].map((s, i) => {
                ...
                return (
                  <div key={s} className="flex-1 space-y-3">
                    <div className={`h-1.5 rounded-full transition-all duration-700 ${isDone ? 'bg-blue-600' : isActive ? 'bg-blue-400 animate-pulse' : 'bg-slate-200'}`}></div>
                    <p className={`text-[10px] font-black uppercase text-center tracking-widest truncate ${isDone || isActive ? 'text-slate-800' : 'text-slate-400'}`}>
                      {s.substring(0, 4)}
                    </p>
                  </div>
                );
